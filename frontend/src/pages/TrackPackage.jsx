...
const BOTTOM_INFO = (info) => [
  { title: 'Sender Information', name: info.sender?.name, addr: info.sender?.city },
    ...
    <div className='flex min-h-screen bg-gray-100'>
      <main className='ml-16 flex-1 p-6'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
          {BOTTOM_INFO(info).map(({ title, name, addr }) => (
            <div key={title} className='bg-white rounded-2xl shadow-sm p-5'>
              <p className='text-xs text-gray-400 mb-2'>{title}</p>
              <p className='text-sm font-bold text-gray-900'>{name || 'N/A'}</p>
              <p className='text-xs text-gray-500 mt-0.5'>{addr || 'N/A'}</p>
            </div>
          ))}
        </div>
      </main>
